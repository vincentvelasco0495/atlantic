...
function buildPanelTitle(dateStart, dateEnd) {
  ...
  if (dateStart === today && dateEnd === today) {
    return `Transaction Details Today (${formatDisplayDate(dateStart)})`;
  }

  if (dateStart === dateEnd) {
    return `Transaction Details (${formatDisplayDate(dateStart)})`;
  }

  return `Transaction Details (${formatDisplayDate(dateStart)} – ${formatDisplayDate(dateEnd)})`;
}
...
export default function TransactionDetailsPanel({ refreshKey = 0 }) {
  ...
  const loadItems = useCallback(async (pageOverride) => {
    ...
    try {
      ...
      const response = await apiGet(`/api/dashboard/transaction-details?${params.toString()}`);
    ...
    <Box className="dashboard-tx-card dashboard-tx-details-card">
        ...
        <Box className="dashboard-tx-heading">
          ...
          <Box>
            ...
            <Typography className="dashboard-tx-subtitle">
              Activity log for login, extend, switch, penalty, and logout events
            </Typography>
          </Box>
        </Box>
      </Box>
      ...
      {!items.length ? (
        <Box className="dashboard-empty-state">
          <Typography className="dashboard-empty-title">No transaction details found</Typography>
              ...
              <TableBody>
                {items.map((item) => (
                  <TableRow hover key={item.id}>
                    ...
                    <TableCell align="right" className="dashboard-tx-cell-num">{item.applied ?? 0}</TableCell>
                    <TableCell align="right" className="dashboard-tx-cell-num">{item.penalty ?? 0}</TableCell>
                    ...
                    <TableCell>{item.bed_name ?? '—'}</TableCell>
